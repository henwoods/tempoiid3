...
fun OiidSidebarLayout(
    ...
    Row(
        modifier = modifier
            .fillMaxSize()
            .testTag("OiidSidebarLayout"),
    ) {
        if (configuration.position == SidebarPosition.Start && sidebarVisible) {
            Surface(
                modifier = Modifier.width(configuration.width),
                color = configuration.backgroundColor ?: OiidTheme.colorScheme.surface,
                content = sidebarContent,
            )

            if (!configuration.overlay) {
                VerticalDivider(
                    color = configuration.dividerColor ?: OiidTheme.colorScheme.outline,
                )
            }
        }
